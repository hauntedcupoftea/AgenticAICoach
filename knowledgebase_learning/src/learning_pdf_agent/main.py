...
from crew import create_learning_pdf_coach_crew
...
def agentic_AI_coach():
    ...
    with st.sidebar:
        ...
        if st.button("Process"):
            try:
                st.sidebar.info(
                    "Processing your documents. This might take a while for large files."
                )
                agent_created = load_data(
                    uploaded_files
                )  # Load documents and create an agent
                if agent_created:
                    ...
                    st.session_state.crew = create_learning_pdf_coach_crew(
                ...
                st.sidebar.error(
                    "An error occurred while processing the documents. Please check the logs."
                )
            ...
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    try:
                        ...
                        assistant_response = st.session_state.crew.kickoff(
                            inputs={"user_question": prompt_with_history}
                        )
